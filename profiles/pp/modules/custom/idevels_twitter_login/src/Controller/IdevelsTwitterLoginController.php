...
class IdevelsTwitterLoginController extends ControllerBase {
  ...
  public function unifiedLoginRegister() {
    ...
    if (!$secret_key|| strlen($consumer_key) == 0)
      throw new \Exception('Missing Twitter secret key.');
    ...
    if (isset($_GET['oauth_token'])) {
      ...
      $request_token = $connection->oauth(
        'oauth/request_token',
        array('oauth_callback' => \Drupal::url('idevels_twitter_login', array(), array('absolute' => true)))
      );
   ...
   * @param $id int Twitter User ID
   * @return \Drupal\user\Entity\User Found user or null if none exists
   */
   ...
   * @param $id int Twitter User ID
   * @param $connection TwitterOAuth API Connection object
   * @return \Drupal\user\Entity\User Found or newly created user
   */
  public static function loadOrRegisterUserByTwitterId($id, TwitterOAuth $connection)
  {
    $foundUser = self::loadUserByTwitterId($id);
    if ($foundUser)
      return $foundUser;
